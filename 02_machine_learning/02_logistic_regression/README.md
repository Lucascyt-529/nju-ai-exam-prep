...
这些指标不是可跳过的扩展。进入本专题前先完成[分类评估主线](../00_model_evaluation/classification/README.md)。训练逻辑回归后必须用验证集概率生成混淆矩阵，报告 precision、recall、F1，并用连续概率生成 ROC 和 AUC；只有训练损失而没有分类指标，不算完成逻辑回归闭环。
