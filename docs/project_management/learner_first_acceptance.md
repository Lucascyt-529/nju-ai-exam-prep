...
| 线性回归 shape 可追踪 | README 与 starter docstring 明确 X、y、w、预测、误差、梯度和损失历史 shape | 通过 |
| 线性回归 demo/check 可定位错误 | demo 展示预测到最小二乘的完整中间量；check 有12项命名核对且 reference 注入全通过 | 通过 |
| 分类指标进入主线 | 数据划分与分类评估为真实子专题；逻辑回归强制使用混淆矩阵、P/R/F1 与 ROC/AUC | 通过 |
| 算法核心模式完整 | 原9题保留，新增滑窗、前缀和、单调栈、堆、链表、树、回溯、贪心和拓扑排序 | 通过 |
| 空文件考试模式 | 线性回归、逻辑回归、kNN、PCA、K-means 只有题面、数据、输出、时间和评分 | 通过 |
| CSV 综合题可分段核对 | demo 展示 CSV 与 shape；check 按10个阶段核对，未完成函数友好停止 | 通过 |
| 简单核对不过度工程化 | 学生日常代码不加入全面防御校验；严格边界仍由 reference/tests 承担 | 通过 |
| 其余入口不是占位页 | NumPy、数据处理、算法题、综合练习和模拟机试均列出实际专题、顺序与启用条件 | 通过 |
| 模拟模式不泄露解题框架 | 新入口只指向开始说明、试卷和数据；计时前禁止查看参考、期望和评分器 | 通过 |
| 旧内容选择性退役 | 旧机器学习跳转页和八个旧总目录页已删除；稳定专题路径继续承载实现与测试 | 通过 |
| 现有学习者代码不被覆盖 | 当前 CSV 学生实现原样保留，仅为空函数补合法占位后纳入最终同步 | 通过 |
| 链接与结构可自动验证 | `test_learner_first_navigation.py`、入口测试和全仓 Markdown 链接测试 | 通过 |
| 状态文档与 Git 同步 | BUILD_STATUS 当前分支修正为 main，详细旧批次移入 history 并保留索引 | 通过 |
...
因此采用“新目录负责稳定学习导航，原专题路径负责内容存储”的方式。旧总目录 README 和只剩跳转作用的旧机器学习 README 已完全删除，不再形成第二套学习路线。这不是未完成迁移，而是明确的最终目录决策。
